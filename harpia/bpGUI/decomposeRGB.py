...
class Properties( GladeWindow, S2iCommonProperties ):
    ...
    def __init__( self, PropertiesXML, S2iBlockProperties):
        ...
        handlers = [
            'on_DRGBBackColorButton_clicked',
            'on_DRGBBorderColorButton_clicked',
            'on_decomposeRGB_cancel_clicked',
        ...
        self.widgets['DRGBBorderColor'].modify_bg(gtk.STATE_NORMAL,t_oBorderColor)        
        ...
        self.widgets['DRGBBackgroundColor'].modify_bg(gtk.STATE_NORMAL,t_oBackColor)

        #load help text
        t_oS2iHelp = bt.bind_file(self.m_sDataDir+"help/decomposeRGB"+ _("_en.help"))
        
        t_oTextBuffer = gtk.TextBuffer()

        t_oTextBuffer.set_text( unicode( str( t_oS2iHelp.help.content) ) )
    
        self.widgets['DRGBHelpView'].set_buffer( t_oTextBuffer )
    ...
    def on_decomposeRGB_cancel_clicked( self, *args ):
    ...
    def on_DRGBBackColorButton_clicked(self,*args):

        t_oColor = self.RunColorSelection()

        if t_oColor <> None:
            
            self.widgets['DRGBBackgroundColor'].modify_bg(gtk.STATE_NORMAL,t_oColor)
    ...
    def on_DRGBBorderColorButton_clicked(self,*args):

        t_oColor = self.RunColorSelection()

        if t_oColor <> None:
            
            self.widgets['DRGBBorderColor'].modify_bg(gtk.STATE_NORMAL,t_oColor)
