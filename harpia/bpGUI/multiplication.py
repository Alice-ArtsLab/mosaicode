...
class Properties( GladeWindow, S2iCommonProperties ):
    ...
    def __init__( self, PropertiesXML, S2iBlockProperties):
        ...
        filename = self.m_sDataDir+'glade/multiplication.ui'
        ...
        handlers = [
            'on_multiplication_cancel_clicked',
            'on_multiplication_confirm_clicked',
            'on_MULTBackColorButton_clicked',
            'on_MULTBorderColorButton_clicked'
            ]
        ...
        self.widgets['MULTBorderColor'].modify_bg(gtk.STATE_NORMAL,t_oBorderColor)        
        ...
        self.widgets['MULTBackgroundColor'].modify_bg(gtk.STATE_NORMAL,t_oBackColor)

        #load help text
        t_oS2iHelp = bt.bind_file(self.m_sDataDir+"help/multiplication"+ _("_en.help"))
        
        t_oTextBuffer = gtk.TextBuffer()

        t_oTextBuffer.set_text( unicode( str( t_oS2iHelp.help.content) ) )
    
        self.widgets['MULTHelpView'].set_buffer( t_oTextBuffer )
    ...
    def on_multiplication_cancel_clicked( self, *args ):

        self.widgets['Properties'].destroy()

    #----------------------------------------------------------------------
   
    def on_multiplication_confirm_clicked( self, *args ):
    ...
    def on_MULTBackColorButton_clicked(self,*args):

        t_oColor = self.RunColorSelection()

        if t_oColor <> None:
            
            self.widgets['MULTBackgroundColor'].modify_bg(gtk.STATE_NORMAL,t_oColor)
    ...
    def on_MULTBorderColorButton_clicked(self,*args):

        t_oColor = self.RunColorSelection()

        if t_oColor <> None:
            
            self.widgets['MULTBorderColor'].modify_bg(gtk.STATE_NORMAL,t_oColor)
