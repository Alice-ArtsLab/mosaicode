...
class Properties( GladeWindow, S2iCommonProperties ):
    ...
    def __init__( self, PropertiesXML, S2iBlockProperties):
        ...
        handlers = [
            'on_subtraction_cancel_clicked',
            'on_subtraction_confirm_clicked',
            'on_SUBBackColorButton_clicked',
            'on_SUBBorderColorButton_clicked'
            ]
        ...
        self.widgets['SUBBorderColor'].modify_bg(gtk.STATE_NORMAL,t_oBorderColor)        
        ...
        self.widgets['SUBBackgroundColor'].modify_bg(gtk.STATE_NORMAL,t_oBackColor)

        #load help text
        t_oS2iHelp = bt.bind_file(self.m_sDataDir+"help/subtraction"+ _("_en.help"))
        
        t_oTextBuffer = gtk.TextBuffer()

        t_oTextBuffer.set_text( unicode( str( t_oS2iHelp.help.content) ) )
    
        self.widgets['SUBHelpView'].set_buffer( t_oTextBuffer )
    ...
    def on_subtraction_cancel_clicked( self, *args ):
    ...
    def on_SUBBackColorButton_clicked(self,*args):

        t_oColor = self.RunColorSelection()

        if t_oColor <> None:
            
            self.widgets['SUBBackgroundColor'].modify_bg(gtk.STATE_NORMAL,t_oColor)
    ...
    def on_SUBBorderColorButton_clicked(self,*args):

        t_oColor = self.RunColorSelection()

        if t_oColor <> None:
            
            self.widgets['SUBBorderColor'].modify_bg(gtk.STATE_NORMAL,t_oColor)
