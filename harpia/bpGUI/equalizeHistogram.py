...
class Properties( GladeWindow, S2iCommonProperties ):
    ...
    def on_equalizeHistogram_cancel_clicked( self, *args ):
    ...
    def on_EQUABackColorButton_clicked(self,*args):

        t_oColor = self.RunColorSelection()

        if t_oColor <> None:
            
            self.widgets['EQUABackgroundColor'].modify_bg(gtk.STATE_NORMAL,t_oColor)
    ...
    def on_EQUABorderColorButton_clicked(self,*args):

        t_oColor = self.RunColorSelection()

        if t_oColor <> None:
            
            self.widgets['EQUABorderColor'].modify_bg(gtk.STATE_NORMAL,t_oColor)
