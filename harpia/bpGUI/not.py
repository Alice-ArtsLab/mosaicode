...
class Properties( GladeWindow, S2iCommonProperties ):
    ...
    def on_not_cancel_clicked( self, *args ):
    ...
    def on_NOTBackColorButton_clicked(self,*args):

        t_oColor = self.RunColorSelection()

        if t_oColor <> None:
            
            self.widgets['NOTBackgroundColor'].modify_bg(gtk.STATE_NORMAL,t_oColor)
    ...
    def on_NOTBorderColorButton_clicked(self,*args):

        t_oColor = self.RunColorSelection()

        if t_oColor <> None:
            
            self.widgets['NOTBorderColor'].modify_bg(gtk.STATE_NORMAL,t_oColor)
