...
class Properties( GladeWindow, S2iCommonProperties ):
    ...
    def on_laplace_cancel_clicked( self, *args ):
    ...
    #----------------------------------------------------------------------
    def on_LAPLBackColorButton_clicked(self,*args):

        t_oColor = self.RunColorSelection()

        if t_oColor <> None:
            
            self.widgets['LAPLBackgroundColor'].modify_bg(gtk.STATE_NORMAL,t_oColor)
    ...
    def on_LAPLBorderColorButton_clicked(self,*args):

        t_oColor = self.RunColorSelection()

        if t_oColor <> None:
            
            self.widgets['LAPLBorderColor'].modify_bg(gtk.STATE_NORMAL,t_oColor)
