...
class Properties( GladeWindow, S2iCommonProperties ):
    ...
    def __init__( self, PropertiesXML, S2iBlockProperties):
        ...
        handlers = [
            ...
            'on_LOGBackColorButton_clicked',
            'on_LOGBorderColorButton_clicked'
            ]
        ...
        self.widgets['LOGBorderColor'].modify_bg(gtk.STATE_NORMAL,t_oBorderColor)        
        ...
        self.widgets['LOGBackgroundColor'].modify_bg(gtk.STATE_NORMAL,t_oBackColor)

        #load help text
        t_oS2iHelp = bt.bind_file(self.m_sDataDir+"help/log"+ _("_en.help"))
        
        t_oTextBuffer = gtk.TextBuffer()

        t_oTextBuffer.set_text( unicode( str( t_oS2iHelp.help.content) ) )
    
        self.widgets['LOGHelpView'].set_buffer( t_oTextBuffer )
    ...
    def on_log_cancel_clicked( self, *args ):
    ...
    def on_LOGBackColorButton_clicked(self,*args):

        t_oColor = self.RunColorSelection()

        if t_oColor <> None:
            
            self.widgets['LOGBackgroundColor'].modify_bg(gtk.STATE_NORMAL,t_oColor)
    ...
    def on_LOGBorderColorButton_clicked(self,*args):

        t_oColor = self.RunColorSelection()

        if t_oColor <> None:
            
            self.widgets['LOGBorderColor'].modify_bg(gtk.STATE_NORMAL,t_oColor)
