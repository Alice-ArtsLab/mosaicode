...
class Properties( GladeWindow, S2iCommonProperties ):
    ...
    def __init__( self, PropertiesXML, S2iBlockProperties):
        ...
        handlers = [
            'on_or_cancel_clicked',
            'on_or_confirm_clicked',
            'on_ORBackColorButton_clicked',
            'on_ORBorderColorButton_clicked'
            ]
        ...
        self.widgets['ORBorderColor'].modify_bg(gtk.STATE_NORMAL,t_oBorderColor)        
        ...
        self.widgets['ORBackgroundColor'].modify_bg(gtk.STATE_NORMAL,t_oBackColor)

        #load help text
        t_oS2iHelp = bt.bind_file(self.m_sDataDir+"help/or"+ _("_en.help"))
        
        t_oTextBuffer = gtk.TextBuffer()

        t_oTextBuffer.set_text( unicode( str( t_oS2iHelp.help.content) ) )
    
        self.widgets['ORHelpView'].set_buffer( t_oTextBuffer )
    ...
    def on_or_cancel_clicked( self, *args ):
    ...
    def on_ORBackColorButton_clicked(self,*args):

        t_oColor = self.RunColorSelection()

        if t_oColor <> None:
            
            self.widgets['ORBackgroundColor'].modify_bg(gtk.STATE_NORMAL,t_oColor)
    ...
    def on_ORBorderColorButton_clicked(self,*args):

        t_oColor = self.RunColorSelection()

        if t_oColor <> None:
            
            self.widgets['ORBorderColor'].modify_bg(gtk.STATE_NORMAL,t_oColor)
