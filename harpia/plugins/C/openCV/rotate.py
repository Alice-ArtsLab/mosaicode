...
# ------------------------------------------------------------------------------
    ...
    def generate(self, blockTemplate):
        ...
        if self.isScalling == "true":
            blockTemplate.functionCall += '		scale = H/(fabs(H*sin(rads(90-abs(block$id$_double_i2)))) + fabs(W*sin(rads(abs(block$id$_double_i2)))));\n' + \
                    '		cv2DRotationMatrix(center,block$id$_double_i2,scale,mat);\n'
        else:
            blockTemplate.functionCall += '		cv2DRotationMatrix(center,block$id$_double_i2,1.0,mat);\n'

        if self.isFilling == "true":
            blockTemplate.functionCall += '		cvWarpAffine(block$id$_img_i1,block$id$_img_o1, mat, CV_WARP_FILL_OUTLIERS, cvScalarAll(0));\n'
        else:
            blockTemplate.functionCall += '		cvWarpAffine(block$id$_img_i1,block$id$_img_o1,mat,0,cvScalarAll(0));\n'
