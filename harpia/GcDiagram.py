...
class GcDiagram(gnomecanvas.Canvas):
    def __init__(self):
        ...
        self.set_scroll_region(0, 0, self.width,
                               self.height)  # must be called everytime a block is moved or inserted with the new scroll region
        ...
        self.grab_focus()
        self.root().connect("event",
                            self.canvas_root_event)  # tem q ser o root() se nao ele pega os eventos antes de todu mundo! =]
        self.connect("event",
                     self.canvas_event)  # tem q ser o root() se nao ele pega os eventos antes de todu mundo! =]
    ...
    def canvas_root_event(self, widget, event=None):
        ...
        if event.type == gtk.gdk.BUTTON_PRESS:  # se temos um clique nao pego por ngm, abortar a conexao
            if event.button == 1:
                ...
                for blockIdx in self.m_oBlocks:
                    self.m_oBlocks[
                        blockIdx].UpdateFocus()  # tricky cos we have a dict not a list (iterating through keys not elements)
    ...
    def __BlockXMLOut(self, t_oBlockIdx, Properties, Network, a_bKeepNonFlowing=False):
        if self.m_oBlocks[t_oBlockIdx].GetState() or a_bKeepNonFlowing:
            props = self.m_oBlocks[t_oBlockIdx].GetPropertiesXML()
            ...
            for t_oConnector in self.m_oConnectors:
                if t_oConnector.fromBlock == self.m_oBlocks[t_oBlockIdx].GetId() and (
                    self.m_oBlocks[t_oConnector.toBlock].GetState() or a_bKeepNonFlowing):
                    Network += '<output id="' + str(t_oConnector.fromBlockOut + 1) + '" inBlock="' + str(
    ...
    def Save(self, a_sFilename=None):  # saving project
        ...
        t_sGcState = "<GcState>\n"
        for blockIdx in self.m_oBlocks:
            t_sGcState += '\t<block type="' + str(self.m_oBlocks[blockIdx].GetType()) + '" id="' + str(
                self.m_oBlocks[blockIdx].GetId()) + '">\n'
